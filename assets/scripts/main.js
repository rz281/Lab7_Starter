...
async function init() {
  ...
  console.log(document.getElementsByClassName("section--recipe-expand")[0].classList);
}
...
function createRecipeCards() {
  ...
  recipes.forEach((rec, index) => {
    ...
    const page = recipeData[rec]['page-name'];
    index > 2 && recipeCard.classList.add('hidden');
    router.addPage(page, function() {
      
      
      document.querySelector('.section--recipe-cards').classList.remove('shown');
      document.querySelector('.section--recipe-expand').classList.add('shown');
      document.querySelector('recipe-expand').data = recipeData[rec];
